...
const authorOutputPath = resolve(root, "src", "generated", "contentPackAuthorIndex.js");
...
const runtimeSource = stripManualCacheTokens(renderContentIndex(
  contentPacks,
  omitAuthorFields(contentCases),
  omitAuthorFields(contentQuickCases),
  contentAdvisors
));
const authorSource = stripManualCacheTokens(renderAuthorIndex(contentCases, contentQuickCases, contentHelperNpcs, contentCast));

if (checkOnly) {
  const currentRuntime = await readFile(outputPath, "utf8").catch(() => "");
  const currentAuthor = await readFile(authorOutputPath, "utf8").catch(() => "");
  if (currentRuntime !== runtimeSource || currentAuthor !== authorSource) {
    throw new Error("content pack runtime index is stale; run npm run content:index");
  }
} else {
  await writeIfChanged(outputPath, runtimeSource);
  await writeIfChanged(authorOutputPath, authorSource);
  console.log(`Content pack runtime index ready: ${outputPath}`);
  console.log(`Content pack author index ready: ${authorOutputPath}`);
}
...
function omitAuthorFields(value) {
  if (Array.isArray(value)) return value.map(omitAuthorFields);
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => !AUTHOR_ONLY_KEYS.has(key))
    .map(([key, child]) => [key, omitAuthorFields(child)]));
}
...
function renderContentIndex(packs, cases, quickCases, advisors) {
...
function renderAuthorIndex(cases, quickCases, helperNpcs, cast) {
  return `// Generated by scripts/build-content-index.js. Author-only. Do not import from the playable bundle.
export const CONTENT_HELPER_NPCS = ${JSON.stringify(helperNpcs, null, 2)};

export const CONTENT_CAST = ${JSON.stringify(cast, null, 2)};

export const CONTENT_AUTHOR_CASES = ${JSON.stringify(cases, null, 2)};

export const CONTENT_AUTHOR_QUICK_CASES = ${JSON.stringify(quickCases, null, 2)};
`;
}
