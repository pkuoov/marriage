...
当前版本主方向调整为 Steam 首发试玩“四案故事集”：一集四通匿名来电，先提供完整试玩体验。玩家作为直播间里的主持侦探，从咨询者的口述、转述、截图和后续补充里听出哪句话没说全。每案仍然线性推进，但四个故事会围绕同一个问题互相照应：好听的身份、关系和流程词，最后到底让谁买单。
...
1. 进入标题页后点击开始。
2. 点击开始接案。
3. 默认进入四案故事集，一集包含四通匿名连线。
4. 听咨询者开口，选择主播下一句最该怎么接。
5. 每段来电只有少数追问方向；有的能摸到瓜心，有的会停在外围。
6. 每次选择会在后台记录路线轴，例如来电人可信度线、材料缺口线、钱流结构线。
7. 最后从几句来电原话里选一句往下接，这一步决定你的结果角度和分享人格，不再一票否决前面的推理。
8. 四案结束后生成故事集总结，展示平均问题揭示率、每案路线和主播倾向。
...
- 故事集总结包含评论区审判墙，把玩家路线、揭示率和本集主题转成可分享的评论区复盘。
- 小程序 web-view 壳已提供，H5 会向壳层发送分享标题、问题揭示率、玩家类型和 `storyKey`。
- 逻辑单测和剧情串读校验覆盖精选集四案结构、每日单案兼容、匿名规则、灰区动机、无废选项和文案连续性。
...
- 把四案故事集从当前 4 个首发试玩案扩到多个完整内容包，每次发版都是一个独立四案故事集。
- 把故事集内容拆成数据包，支持 Steam 发版内容包和旧集回看。
