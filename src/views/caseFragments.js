export function tutorialGuideLine(brief, step) {
  ...
  const guides = {
    ...
    scene: "孟姐提示：客户叙述里至少有一个版本讲得顺但不完整。选一个话术问下去，把不自洽之处记成矛盾。",
    testimony: "孟姐提示：通话摘录不是为了让人认错，而是让台词和材料互相咬住。优先问含糊话、半真话和防御话。",
    evidence: "孟姐提示：后台资料只能证明局部事实。把时间线、钱和动机放在一起看，才能避免被单张截图带跑。",
    accusation: "孟姐提示：阶段判断前至少抓到一处矛盾。没有材料支撑的同情，也可能误伤另一方。"
  };
