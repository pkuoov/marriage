import { applyDifficultyProfile } from "./difficulty.js?v=0.20.42";
import { DEFAULT_STORY_PACK_KEY, storyPackCaseCount, storyPackForKey } from "./storyPacks.js?v=0.20.42";
...
function inferRouteAxis(option = {}, scene = {}) {
  const text = `${option.question ?? ""} ${option.answer ?? ""} ${scene.version ?? ""}`;
  if (/你当时|你自己|你妈|家里|心疼|怕|委屈|表现|抢署名|要不要|你是不是|你有没有|起疑|绕着|不踏实|怎么接|怎么回|怎么理解|自己人|拦过|改口|为什么先答应|更慌/.test(option.question ?? "")) return "caller-credibility";
...
function inferRouteTone(option = {}, scene = {}, sceneIndex = 0) {
