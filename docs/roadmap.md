...
- 完整案件 JSON loader 入口已接入：`CONTENT_CASES` 会记录每案 `runtimeContentStatus`；前三案已切到 `runtime-loaded` 并由内容包覆盖模板字段。
...
- 内容包管线仍是 P0 架构债：manifest 和 loader 入口已接运行时生成索引，前三案已从 JSON 读取完整内容；其余案件仍是 `metadata-only` 压力包，完整台词仍来自 `src/caseEngine.js`。下一步继续逐案切到 `runtime-loaded`。
