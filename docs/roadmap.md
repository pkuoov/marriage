...
- 现场压力模型第一层已拆成纯函数：听众耐心、弹幕跑偏/压住、连线人防备和人物表情钩子开始读取同一份压力画像；追问语气和材料命中/误指也开始从这里生成现场反应。
...
- 资料操作的基础版式与回流反馈已稳定，现场压力模型第一层已接入；下一步把压力画像接进结算和终局路线画像。
- 现场压力稳定后，再回收材料圈点、原话选择和路线画像，让终局不是只看揭示率。
