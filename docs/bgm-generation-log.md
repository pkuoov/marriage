...
## 2026-09-06 · 三首新增素材工程选剪与接入

- 三个目录共五份 WAV 已核对。第一幕选 `late night 1` 的 48.016 秒循环，追索选 `1.wav` 中后段的 72 秒循环，尾声选 `quiet dawn` 的 90.288 秒循环。
- 三首均已接入，11 个 BGM 槽位均有可播放文件；两个未选 take 已另做等响度循环备选。
- 本轮只有信号分析与工程验证，无法直接接收音频输入，没有进行耳听签收。第一幕同旋律关系、尾声固定脉冲与追索拥挤感仍需核对，保持 `auditoryReview: pending`。
- [完整选片理由、裁切点、响度与场景分配](review-archive/bgm-selection-2026-09-06.md)。

## 2026-09-05 · 追索提示词 v2 与新增候选

- 用户反馈追索音乐多次生成都比较噪；旧低音脉冲版和刷鼓备用版停用。新提示以 84 BPM 电钢琴独奏为主，柔和钢琴独奏为备选，不再叠鼓、贝斯和质感层。具体噪声类型尚未测量，提示词调整效果待新生成试听。
- 收到第一幕 `late night 1.wav`、`late night 2.wav`，以及尾声 `quiet dawn.wav`，原件保留在各自 `assets/audio/source/bgm-*/` 目录。
- 三份源文件已做格式、响度、真峰值检查；制作全曲统一响度试听版，输出到 `assets/audio/review/`。听感、同旋律要求和循环乐句尚未验收，catalog 保持 planned。
- 当前提示词与实测表见 [缺曲补全单](udio-missing-bgm-prompts-2026-09-05.md)。
