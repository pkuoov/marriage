...
2. 案 3 饭局停顿：由咨询者林本人发问；逐字稿采用当前饭局原句，男方停筷和回答交给剧情，不在问句干声里填十几秒空白。
3. 案 4 垫款私聊：采购经办同事应像在赶项目，不像预谋反派；不要使用主播林旭阳的声线。
4. 案 4 供应商补话：把“老规矩”说得普通，关键性由上下文产生。
5. 案 1 怕你离开：不阻塞当前试玩，可随同一录音批次补齐。开播使用现有平台提示音，不录制人声倒数。
...
- `npm run audio:voice -- prepare all` 把五段固定参考录音稿写进本地忽略目录，演员照稿录制同目录的 `reference.wav`；
