...
- 现场压力模型第一层已拆到 `src/runtime/livePressure.js`：听众耐心、弹幕跑偏/压住、连线人防备和人物表情钩子开始由同一份纯函数画像生成。
...
- 事实边界归位已影响最终收话余味和故事集终局：放稳、放早再收回、仍未归位，会得到不同边界标签和评论区反应。回流材料的命中/误指也已接进案间余味和弹幕压力；下一步继续把更多追问语气和材料误指接进现场压力模型。
...
2. 现场压力模型：第一层已接入，继续让追问语气、材料误指和回流结果共同影响听众耐心、连线人防备、弹幕跑偏。
