...
- **正式 BGM**：六个槽位已接入；继续按 `docs/udio-bgm-production-prompts-v2.md` 完成最终追问、单案回看和天亮前三首，原始 WAV 仍须经统一裁循环、归一和转 OGG。
...
- 九槽位 BGM 已提升为 P0；地点环境底和非关键语音仍先听真人录音与真机混音结果再排产。

### P2｜试玩稳定后的扩展

- 嘉宾位 P3、完整计策系统、全量时间线拼装、多点圈选。
- 第二故事包与新增案件生产管线。
- 动态弹幕瀑布、虚拟手机检视和随身线索夹；先做交互、性能和遮挡设计，再决定是否实现。

### 已冻结、不要返工

- 四案 `neutral / guarded / pause` 已统一为暖色三分之二侧脸、简化五官与真透明像素资产；眼部光带、遮罩和上半脸重阴影已经废止。
- 当前 JSON 是台词真源；全量文字剧本、导演版和纯故事台本均由生成器同步，禁止手改生成文件。
...
当前下一步只做：**四案完整真人 playtest 备场与报告归档**。现场压力、材料圈点、案后回流、事实边界、内容包和桌面壳都已接入第一版；冻结期内不和 AI 问答、新故事扩写同时推进。
...
2026-07-04 至 2026-07-06 已完成的支撑项（剧情与顾问线，八批处理稿全部落地）：

- 引语公平性收口：四条最终原话全部有无门控出处，`quotePickCandidates` 与出货同步，引语映射账本进 `qa-report.md`；「引语出处不得被分支门控」已入 `detective-plot-coupling-review` 公平性检查。
- 四案悬疑改造：假解全部有真料喂养（店里统一话术／财务延后通知）、来电人春秋笔法开场可抓（会员号订座／她先提流水）、收麦一律三段式（他的链条／她的链条／今晚定不了）、案 4 承担包级元反转、身份期票种子进评论墙。
- 案 1 案情级转折：S2 将八万信用卡拆成不到三万共同消费、约一万五男方男装、至少三万五未说明（「反正不是乱来的钱」）＋往期账单板（每月 8 号固定私人入账在七月中断）；信用卡三万五与银行五万贷款/49,800 转出严格分开，拆东墙／多线／化债只作为开放读法；分享问题是唯一邀请猜测面。
- 实测八项修复：S1 误加分期关键项改闲聊闪避、回流补图第三人称化、谜语腔 feedback 清理、氛围填充句全部清空、现场压力卡去黑话、运行时文案池纳入禁词机械扫描、账单板 `materialRows` 结构化、`showsCard` 开场材料实物化。
- 体量扩充：四案 40 条署名闲聊问答（`casualQuestions`，与关键选项零回收）、证言更新（`revisedVersion`，案 1 圈板后当场改口找不同）、材料板反应台词（`reactionLine`，案 1 四选项全配）。
- 麦外来信：顾问注册表四人（赵律师／周会计／小林老师／张法医）、`advisorNotes`（案 1 赵律师、案 4 周会计，效力/侦查铁线机械化——顾问文本禁"圈/那一栏/哪一块"）、`respondentNote`（案 2、案 4，对方的单向剪辑留言）。
- 编剧方法论成体系：`case-scriptwriting` 新增语言规则（口语句法＋接话头）、六运算制造法（真相账本→疑点→问题点→转折点）、扩量 Playbook 九法、顾问铁线与证据委托规则；`coupling-review` 新增诡计分层、伏笔账本、双源规则、悬疑检查、A/B 故事、场景转向、信息差配置；三 skill 分工闭环（造／验／玩）。

2026-07-07 至 2026-07-08 已完成的支撑项（第 10-14 批及美术 V2 接线已落地）：

- 第 10 批：案 3 抢答存款证明链路收束，`showsCard` 铺开案 2-4，案 2 吹风机听觉线索入场，案 1 材料误指后的 `pityLine` 侧向提示落地。
- 第 11 批：四案扩容到 7 场 3 板，材料中置、立场快照和麦外群像完成，场景体量从"长台词"转为多种阅读动作。
- 第 12 批：证据委托上线，顾问从彩蛋变成可选诊断路线，四案均有委托回单且早于最终原话。
- 第 13 批：戏剧特许四味落地，案 1 lurker、案 2 受害者分裂、案 3 靠窗韵脚、案 4 "老规矩"一词两义额度已满。
- 第 14 批：顾问家人网和家常配额落地，关系只提供口吻和熟人感，不进入专业核心判断。
- 美术 V2 接线：来电人立绘变体按 `neutral/guarded/pause` 映射，缺图回退 neutral；旧 prompt 包封存，V2 验收标准已入 `docs/ui-art-asset-review.md`。
...
- 九种运行时音乐节拍均有正式 BGM；普通连线切压力层时同速同调，首页 30 秒不显重复，所有案内曲不抢中文对白。
...
### 剧情优化清单（2026-07-08 冻结刷新，第 10-14 批已销账）

已完成支撑项：

- 第 10 批：抢答存款证明、`showsCard` 铺开、吹风机听觉线索、`pityLine`。
- 第 11 批：扩容 7 场 3 板、立场快照、麦外群像。
- 第 12 批：证据委托。
- 第 13 批：戏剧特许四味。
- 第 14 批：家人网。
- 第 15 批前置接线：立绘变体接线与 V2 验收准备。
- P1 像素立绘：案 1/2/3/4 的 `neutral / guarded / pause` 已完成母版、发运版、运行时三态切换与自动验收；2026-07-18 复审后又统一为暖色三分之二侧脸，取消常驻遮罩和眼部光带。
...
- 四案均已补 `neutral / guarded / pause` 像素差分并随现场表情切图；旧半写实 V2 和眼部光带候选只保留为制作记录，不再进入运行时。
...
1. 剧情管线八批已全部落地，案 1 已是旗舰形态——当前最高优先级是按 `docs/playtest-report-template.md` 打一轮完整真人 playtest，用实弹数据决定"剧情优化清单"里待铺开项的顺序；共性问题照旧进 skill 和自动检查。
