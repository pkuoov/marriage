...
- 替换四案背景：信用卡/社保账单、理发表格/会员卡、资料核验桌、财务报销办公室。
- 增加四个匿名来电人半卡通立绘，不再只复用命名 NPC 约会/职场 archetype。
- 把运行时立绘从低细节 SVG 升到透明 PNG，并补紧张、停顿、防备、松动等表情差分。
...
- 事实边界归位已影响最终收话余味和故事集终局：放稳、放早再收回、仍未归位，会得到不同边界标签和评论区反应。回流材料的命中/误指也已接进案间余味和弹幕压力，压力画像、材料圈点和原话选择已进入结算和终局路线画像。
