...
- 当前体验仍偏手机点击。
- Steam / Steam Deck 需要键盘、手柄、焦点环和默认焦点。
- 方向键 / WASD 切换选项，Enter / Space 确认，Esc / B 返回，Tab / Y 回看。
...
1. 先建内容包 schema 和 `content/packs/steam-demo-01/`。
2. 再拆 `app.js` 的 recap / route / scene runtime。
3. 做键盘焦点和桌面壳最小闭环。
4. 扩充模板池到 9-12 个。
5. 做受控自由追问本地 matcher。
6. 最后接 AI router 和训练标注。
