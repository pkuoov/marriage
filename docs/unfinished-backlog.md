...
- 完整案件 JSON loader 入口已接上：生成索引会输出 `CONTENT_CASES`，`runtime-loaded` 案件可通过 `src/runtime/contentCase.js` 覆盖模板字段；当前 demo 前三案已切到 `runtime-loaded`，其余一案仍保持 `metadata-only`。
...
- 2026-07-02 复查结论：`content/packs/steam-demo-01/cases/*.json` 不能再做影子资产。前三案已是运行时台词来源；其余 `metadata-only` 案件仍只能写策划压力包，`npm run verify:pack` 会阻止它们夹带运行时字段。
...
- 当前 demo 包前三案完整台词、追问、材料判定和结算已迁入内容包；其余一案仍写在 `src/caseEngine.js`，下一步继续逐案迁移。
