...
| PACK-005 | Runtime content schema | Runtime-loaded cases must expose playable nested content: opening dialogue, scene question pairs, exactly one core question per beat, material marking, investigation backflow, deep follow-up, conclusion, share copy, and truth text. | 内容包看着完整，实际缺少可被引擎消费的台词、选项、材料、回流或结算字段。 |
| BROWSER-SMOKE | Playable route replay | `npm run smoke:browser` opens the offline playable and replays perfect, outer-then-core, material-miss, and keyboard-perfect routes to the recap. | 离线包语法能过但真实按钮无响应、import alias 丢失、后台回流页崩溃、材料误圈路线死路，或键盘焦点无法实际通关。 |
