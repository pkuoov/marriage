...
| PACK-005 | Runtime content schema | Runtime-loaded cases must expose playable nested content: opening dialogue, five scene beats, exactly one core question per beat, at least two readable materials, investigation backflow, caller omission, third pressure, deep follow-up, conclusion, share copy, and truth text. | 内容包看着完整，实际缺少可被引擎消费的台词、选项、材料、回流、结算字段，或体量退回短问答。 |
| BROWSER-SMOKE | Playable route replay | `npm run smoke:browser` opens the offline playable and replays perfect, outer-then-core, material-miss, keyboard-perfect, and gamepad-perfect routes to the recap. | 离线包语法能过但真实按钮无响应、import alias 丢失、后台回流页崩溃、材料误圈路线死路，或键盘/手柄焦点无法实际通关。 |
