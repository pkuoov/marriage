...
主案 JSON 分两种状态：`metadata-only` 只记录策划压力系统；`runtime-loaded` 会接管该案完整台词、追问、材料判定、满格深问和复盘文案。
...
- `evidenceChecks`：材料圈点，至少一个；每项包含 `id`、`title`、`prompt`、`material`、`options`。选项数量与正确项按实际材料玩法校验，成立项须有对应事实，不把多选、圈行和单项推进混为一谈。职场/流程案可以用两份材料制造流程压力，例如先看审批图，再看供应商返款入口。
...
- `deepFollowup`：当前主案达到相应门槛后出现的追问，包含 `question`、`answer`、`note`。它不是奖励提示，要像主播顺着已经听到的事实多问了一句。
...
2. 按当前模式写完整交流：接通、陈述、逐句询问、材料、必要转场与收麦。轮数随事实披露需要决定，结案不强制凑复盘页。
...
- `questionSequence` 是事实依赖顺序。逐句回放一屏一条完整原句，提供“询问这句／不询问”；选询问后在对白中播放实际问题与回答，再回到原句。单项继续不是关键选择。
...
- `timelineSort.cards / correctOrder` 引用同一组事件。只有正确排列才能推进；恢复存档同样重验正确性，不能只凭“已提交”放行。
