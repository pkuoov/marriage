...
- `$manifest.nightShell.prologue.lines[2]` 吃饭没有?你的包在椅背上。汤在冰箱,是给你热的,不是给你放过期的。案卷也在包里,收播后再看。
...
- `$case.hostDisclosure.lines[0]` ……老林，喝口水。三十秒，不着急。
- `$case.hostDisclosure.lines[0].text` ……老林，喝口水。三十秒，不着急。
