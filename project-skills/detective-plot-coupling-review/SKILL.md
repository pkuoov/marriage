...
- Multi-scene quota (场景配额, design: `docs/multi-scene-rashomon-design.md`): every case needs at least two non-console investigation surfaces (city day / doorstep / observe / document room). Licensed co-presence (特许同席) is optional and survives only when it adds an independent contradiction that no caller statement or existing material has already supplied. The livestream is one stage in the night, not the only stage.
...
- Removing a licensed co-presence scene means removing its witnesses and evidence delivery too. Do not relabel the scene as `visit` and preserve a random bystander who recognizes the show, volunteers a perfect line, and repeats a fact the caller already admitted.
