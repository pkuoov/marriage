...
- Night-B first-conflict law: every reachable opener must include `firstConflict.hostLine` and `firstConflict.callerLine`, and the host question must test the distinct load-bearing issue carried by that item. Changing only `opener.line`, or reusing one generic confrontation across items, fails the bring-back law.
