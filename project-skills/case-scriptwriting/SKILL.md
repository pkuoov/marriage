...
人物需要说的是自己为什么要录、为什么拒绝、为什么继续谈，以及现在希望对方做什么；不要让人物替导演报机位、替系统解释按钮、替作者声明身份边界。`把镜头压到桌面`、`今天只理桌上这些`、`我不是诉讼代理人` 都可能事实正确，却像场面调度或角色卡。改写时优先落回当场关系：`先录下来，省得后面有人不认`、`今天先聊已经摆出来的事`、`我们还没到起诉离婚的程度`。

同样，人物不能无缘无故朗读保全清单。只有材料正面临遗失、删除、争抢或来源争议时，才让人物说“别删／先留好”；没有现实威胁时，`原手机留好`、`完整流水别删` 只是作者借角色复述工作步骤，应删除。律师、鉴定顾问和主播还要服从场景保密：当对方在场时，只谈当前能公开的事；具体调查、检测或诉讼操作留到私下咨询，不能当着对方教另一方下一步怎么查。

按钮和台词必须承认已经发生的动作。录像已经关掉，按钮只写`继续谈`，不能再写`关录像，继续谈`；两个选项若会进入同一内容、得到同一反馈，就不是选择，应合并或让后果真正分开。材料也一样：点击“查看”必须打开可读原件，出示成功后应离开当前待选区，但仍可在历史材料中查到；只把`查看`换成`已查看`不算交互反馈。
...
**电报式狠话是判断 AI 腔的重要标尺。** AI 不只会把话写长，也常为了显得利落、强势或像短剧钩子，把一个人的解释和请求压成两三截口号：`反正不是乱来的钱。别拖，今晚先转。` 每一截单看都通，连起来却没有人在对谁解释、为什么着急、希望对方具体帮什么，只像三条推送文案。审查时把标点去掉：如果各分句都能直接拿去做按钮、标题或字幕，人物关系和当场说服动作却消失了，就按 AI 腔返工。

返工不是机械加长，而是补回这一刻真正存在的人际动作：解释用途、哄对方、催促、求情、施压、给承诺或留退路，至少让其中一层落到具体对象上。比如可以写成：`我又不是拿去乱花，奖金这两天就发了。你先帮我把这期卡还上，别真拖到逾期，今晚能转给我吗？` 高压峰值可以突然只剩一句命令，但前文必须已经交代对象、原因和关系压力，而且这种骤短要符合人物正在失控或夺权的状态；不能让整场所有人都靠惜字如金显得“有力”。
...
- Bad: 「问过一次。他说奖金快发。我再问，他就把话岔开了。」
- Good: 「我问过。他当时愣了一下，只说奖金这两天就发。后来我又追了一句，他没接，直接催我今晚把钱转过去。」

The extra words are not filler: `当时` separates remembered event-time from the caller's present narration, and `后来` makes the second question a consequence in the same recalled exchange. More summary-removal and retelling shapes live in [spoken-corpus-patterns.md](references/spoken-corpus-patterns.md).
...
主案、快案和序章里凡是承担调查的段落，都以“一个事件阶段”为单位，不以单个问句为单位。先让当事人把同一阶段的若干相连片段讲完，允许连续翻过几屏；全部讲完以后，玩家才进入同一批原话的集中回放，并实际处理两处以上。禁止每播一小段就立刻反驳一次，再让人物补下一小段；那会把事件说法拆成问卷，也会让来电人永远没有机会建立一套可被推翻的完整版本。

- 一个阶段通常包含二至四段相连陈述，共享同一时间、物件或眼前诉求；下一段必须接住上一段留下的问题。为了凑数量而并排无关事实，不算完整阶段。
- 阶段首次播放可以把长陈述拆成数个自然口语屏，但这只是翻页，不是交互切口。最后一屏结束前，不开放反驳按钮，不让主播替玩家提前总结矛盾。
- 集中回放必须保留阶段内全部已播原话。一个完整阶段默认让玩家至少完成两次实际回放处理，对应两个主要问题点；玩家先处理这两处承重原话，人物才有理由换一整套解释或进入下一阶段。若一处物证与原话的碰撞足以让整套说法倒下，可以只设一个决定性承重点；若真正承重问题超过两个，就按时间、物件或责任拆成两个阶段，不能把三四项并成审计清单。普通追问可以补足人物反应和信息，但不能替代尚未处理的承重片段。
- 挂断、来电人明确暂停、必须先看的材料、立场快照和新一夜是阶段硬边界，不得为了凑一组跨过去。硬边界前只有一段陈述时，允许单段成阶段，但这段须能独立讲清一个局部事件，回放仍至少处理两处。
- 本律只管调查核心段。问候、接通确认、材料签收、关怀、判词和收麦尾声按人物当场需要写，不强制凑成多段回放。
- 证词墙的 A 与 A′ 本身已经是完整阶段：A 必须整套讲完后再盘问。两处主要矛盾被指出（或一件决定性物证击穿整套说法）以后，人物必须换一整套 A′，不能逐句给 A 打补丁。A′ 首次出现时要有明确的“改口陈述”过场，再让人物连续讲完；玩家第二次盘问之后，才进入事实边界或下一阶段。

- **分段披露律**：一宗快案分成二至三轮“整段听麦 -> 原句回放 -> 短问对质”。每轮先播完一个能独立理解的局部经历，再把这段拆成句条；未来轮次才会出现的人、材料和矛盾必须隐藏。对质逼出的新事实可以成为下一轮陈述前提，不能让玩家在事实尚未出现时预选答案。
- **三种播放律**：同一阶段的 `statement` 必须有三种听法。首次听麦时按原顺序连续播放，可以自然拆成几屏，但要等这一阶段全部说完，来电人仍占主场；回放时只调用已播原句的逐句切分，不新增、不摘要；打断后只播放玩家按住的 `sourceAnchor` 所对应的主播 `question` 和人物攻防。每个阶段首次听麦前必须用全屏大字进入「来电人陈述」，集中回放前再用全屏大字进入「逐句追问」；人物换掉整套说法时，再用同样的全屏换挡进入「改口陈述」。大字只负责舞台换挡，不解释规则或暗示正确句，不能每播一句都跳一次。若白天材料会把 `version` 换成 `revisedVersion`，必须另给 `revisedSourceAnchor`，并逐字落在实际播出的改口陈述里；不能让回拨后只剩错误句可点。集中对质结束后的新长答回到下一段听麦，不能一直停在一问一答节拍器里。每次连线至少有两个调查阶段；硬边界导致单段阶段时，按上面的例外处理，不用塞无关台词凑轮数。
